...
public class CompletionEndDate {
	...
	public static void main(String[] args) {
		...
		for(int n=0;n<normalCodeList.size();n++){
			...
			if(CreateTable.existTable(windCode)){ 
				...
				int lastOne = intDateList.get(intDateList.size()-1);
				//System.out.println(lastOne);
				intDateList = new LinkedList<Integer>();
				for(int i=0;i<13;i++){
					intDateList.add(lastOne+i+1);
				}
				...
				for(int i=0;i<intDateList.size();i++){
					...
					if(intDate < 20160519){
						...
						if(ticksList.size() != 0)
						System.out.println("!!!!!!!!数据: "+ticksList.size());
